    ...
    public class RpcEndpointList : TabPage
    {
        ...
        public RpcEndpointList (RpcLibrary Library, TabManager Manager)
        {
            ...
            m_Listview.DoubleClick += new EventHandler((object obj, EventArgs e2) =>
            {
                ...
                var id = selectedRow.InterfaceId.ToString();
                var version = selectedRow.InterfaceVersion.ToString();
                var server = m_Library.Get(id, version);
                if (server == null)
                {
                    MessageBox.Show("Unable to locate RPC server " + id + ", " + version);
                    return;
                }
        ...
        public void Build(string Name, List<RpcEndpoint> Endpoints)
        {
            ...
            {
                Trace.TraceError("Unable to retrieve RPC endpoint list: " + ex.Message);
            }
        }
        ...
        private void ContextMenuNewClient(object Sender, EventArgs Args)
        {
            ...
            var id = endpoint.InterfaceId.ToString();
            var version = endpoint.InterfaceVersion.ToString();
            var server = m_Library.Get(id, version);
            if (server == null)
            {
                MessageBox.Show("Unable to locate an RPC server definition for UUID " + id +
                    " and version " + version);
                return;
            }
