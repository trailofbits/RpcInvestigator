...
namespace RpcInvestigator
{
    ...
    public class RpcServerData
    {
        ...
        public bool Add(RpcServer Server)
        {
            ...
            {
                m_Entries.Add(
                    Server.InterfaceId, new List<RpcServerEntry>());
            }
            foreach (var server in m_Entries[Server.InterfaceId])
            {
                if (server.InterfaceVersion.Equals(Server.InterfaceVersion))
                {
                    return false;
                }
            }
        ...
        {
            if (!m_Entries.ContainsKey(InterfaceId))
            {
            ...
            if (m_Entries.Remove(InterfaceId))
            {
                Trace(TraceLoggerType.RpcLibrary,
                    TraceEventType.Verbose,
                    "Library: Removed RPC server ID " +
                    InterfaceId.ToString());
                return true;
            }
            Trace(TraceLoggerType.RpcLibrary,
                TraceEventType.Verbose,
                "Library: Failed to remove RPC server ID " +
                InterfaceId.ToString());
            return false;
        }
        ...
        public bool Remove(Guid InterfaceId, Version InterfaceVersion)
        {
            ...
            foreach (var server in m_Entries[InterfaceId])
            {
                if (server.InterfaceVersion.Equals(InterfaceVersion))
                {
                    if (m_Entries[InterfaceId].Remove(server))
                    {
                        Trace(TraceLoggerType.RpcLibrary,
                            TraceEventType.Verbose,
                            "Library: Removed RPC server ID " +
                            InterfaceId.ToString() + ", version " +
            ...
            Trace(TraceLoggerType.RpcLibrary,
                TraceEventType.Verbose,
                "Library: Unable to locate RPC server ID " +
                InterfaceId.ToString() + ", version " +
                InterfaceVersion.ToString());
            return false;
        }
        ...
        internal Dictionary<Guid, List<RpcServerEntry>> Get()
        {
            return m_Entries;
        }
    ...
    public class RpcLibrary
    {
        ...
        public Dictionary<Guid, List<RpcServer>> GetServersWithMultipleVersions()
        {
            ...
            var servers = new Dictionary<Guid, List<RpcServer>>();
            foreach (var kvp in all)
            {
        ...
        public List<RpcServer> Find(RpcLibraryFilter Filter)
        {
            ...
            switch (Filter.FilterType)
            {
                case RpcLibraryFilterType.FilterByKeyword:
                    {
                        var all = Get(); // expensive: deserializes all servers!
